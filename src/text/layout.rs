...
use super::{FontDescriptor, TextStorage};
use piet_common::{
    kurbo::{Line, Point, Rect, Size},
    Piet,
};
use piet_common::{
    Color, FontFamily, PietText, PietTextLayout, RenderContext, Text as _, TextAlignment,
...
impl<T> TextLayout<T> {
    ...
    //     TextLayout {
    //         text: None,
    //         font: FontDescriptor::new(FontFamily::SYSTEM_UI).with_size(15.0),
...
impl<T: Debug> std::fmt::Debug for TextLayout<T> {
...
impl<T: From<&'static str>> Default for TextLayout<T> {
    fn default() -> Self {
        Self::from_text("")
    }
}
